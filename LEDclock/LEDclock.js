...
var LEDclock = (function(){
  ...
  function changeState(digitLED, digitDataChunks, index){
    ...
    for(var i = 0, max = partsOfLED.length; i < max; i++){
      ...
      if(state===0){
        part.style[borderTarget] = 'black';
      }
      
      if(state===1){
        part.style[borderTarget] = 'red';
      }
    }
  }
  ...
  function getTimeAndUpdate(){
    ...
    if(isPM){
      ...
      document.getElementsByClassName('am-pm')[0].style.backgroundColor = 'red';
    }

    if(!isPM){
      hours = hours.toString();
      document.getElementsByClassName('am-pm')[0].style.backgroundColor = 'black';
    }
